...
///*AAC解码*/
typedef enum {
    SET_DEFAULT_NOTING       = 0,
    SET_SPECIFIC_PARAMETERS  = 1,
} _MUSIC_DEC_CONFIG;

typedef struct __AAC_DEFAULT {
    u32 samplerate;
    u32 nchannels;
    u32 bitrate;
} AAC_DEFAULT;
...
#define SET_CLEAR_STATE			0xFD
#define SET_RESET			0xFE
...
extern decoder_ops_t *get_dts_ops(void);
extern decoder_ops_t *get_amr_ops(void);
extern decoder_ops_t *get_m4a_ops(void);
extern decoder_ops_t *get_udsbc_dec_ops();
#endif
