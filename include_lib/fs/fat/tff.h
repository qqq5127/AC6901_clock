...
#define LFN_MAX_SIZE    512   	//不能超过512		//必须4byte对齐
...
FRESULT f_seek(FIL _xdata *fp, u8 type, u32 offsize);
u16 f_read(FIL _xdata *fp, u8 _xdata *buff, u16 btr);
FRESULT f_readnextdir(FATFS _xdata *fs, DIR *dj, DIRINFO _xdata *dinfo, FS_NAME _xdata *fs_n);
void load_dinfo2dj(FATFS *fs, DIR *dj, DIRINFO *dinfo);
void  f_opendir(FATFS _xdata *fs, DIR _xdata *dj) ;
FRESULT  f_mount(FATFS _xdata *fs, u32 bootsect, char drive_sel) ;
FRESULT f_mount_api(FATFS _xdata *fs, u32 bootsect, char drive_sel);
FRESULT f_open(FATFS _xdata *fs, FIL _xdata *fp, char *path, u8 mode);
u8 f_tell_status(FIL _xdata *fp);
...
u32 clust2sect(FATFS _xdata *fs, u32 clust);
void st_clust(u8 *dir, u32 cl);
FRESULT sync_window(SWIN_BUF *win_buf);
FRESULT  move_window(u32 sector, SWIN_BUF *win_buf);
u32 get_fat(FATFS _xdata *fs, u32 clust, SWIN_BUF *win_buf);
u32 get_fat_by_dir(FATFS *fs, DIR *dj, u32 clust, SWIN_BUF *win_buf);
u32 get_cluster(FIL _xdata *fp, u32 clust, SWIN_BUF *win_buf);
...
FRESULT check_fs(FATFS _xdata *fs, u32 sec);
void get_dir_info(const u8 _xdata *dir, DIRINFO _xdata *dinfo);
bool dir_next(FATFS _xdata *fs, DIR _xdata *dj, bool stretch);
FRESULT f_open_dir(FATFS _xdata *fs, DIR *dj, char *path);
...
bool decode_lfn(char *target_p, char *source_p, u16 max_copy); //const char *file_type)

int find_word_pos(u8 *buf, u16 buf_len, u16 key, u8 find_type);
void fs_lfn_deal(FATFS _xdata *f_s, FIL *f_p);
void deal_file_info(char *tpath, LONG_FILE_NAME *disp_file_name, LONG_FILE_NAME *disp_dir_name);
